...
def Upload_dataset():
    ...
    songs= [Song(id_songs=row['id_songs'], name_song=row['name_song'], danceability=row['danceability'], duration_ms=row['duration_ms'],release_date=row['relase_date'], album_id=Album.objects.get(album_id=row['album_id'])) for i, row in df.iterrows() if not Song.objects.filter(id_songs=row['id_songs']).exists() ]
